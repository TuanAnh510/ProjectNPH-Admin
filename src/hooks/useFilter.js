...
const useFilter = (data) => {
  ...
  const [searchCategoryBlog, setSearchCategoryBlog] = useState("");
  ...
  const serviceData = useMemo(() => {
    ...
    if (searchCategoryBlog) {
      services = services?.filter((search) =>
        search?.title
          ?.toLowerCase()
          ?.includes(searchCategoryBlog?.toLowerCase())
      );
    }
    ...
    if (searchBlog) {
      services = services?.filter((search) =>
        search?.title?.toLowerCase()?.includes(searchBlog?.toLowerCase())
      );
    }
    // order filtering
    if (status) {
    ...
    if (shipping) {
      services = services.filter((ship) =>
        ship?.transportunitname.toLowerCase().includes(shipping.toLowerCase())
      );
    }

    //region filtering
    if (region) {
      services = services.filter((re) =>
        re?.regionname.toLowerCase().includes(region.toLowerCase())
      );
    }
  ...
  const handleSubmitCategoryBlog = (e) => {
    e.preventDefault();
    setSearchCategoryBlog(categoryBlogRef.current.value);
  };
  const handleSubmitBlog = (e) => {
    e.preventDefault();
    setSearchBlog(blogRef.current.value);
  };
  ...
  const handleUploadProducts = () => {
    if (newProducts.length < 1) {
      notifyError(t("PleaseSelectaValidCSV"));
    } else {
  ...
  const handleUploadMultiple = (e) => {
    ...
    if (selectedFile.length > 1) {
      ...
      notifyError(t("PleaseSelectaValidJson"));
    }
  };
