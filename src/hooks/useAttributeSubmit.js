...
const useAttributeSubmit = (id) => {
    ...
    try {
      ...
      if (!id) {
        if (variants.length === 0) {
          notifyError(t("notifyErrorMinimum"));
          return;
        }
      }
      const attributeData = {
      ...
      if (id) {
         await AttributeServices.updateAttributes(id, attributeData);
        setIsUpdate(true);
        ...
        setServiceId();
         ...
         await AttributeServices.addAttribute(attributeData);
        setIsUpdate(true);
        setIsSubmitting(false);
  ...
  const onSubmits = async ({ name }) => {
    try {
      setIsSubmitting(true);
      if (id) {
         await AttributeServices.updateChildAttributes(
        ...
        closeDrawer();
         ...
         await AttributeServices.addChildAttribute(
