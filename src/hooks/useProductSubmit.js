...
const useProductSubmit = (id) => {
  ...
  const [tapValue, setTapValue] = useState(t("BasicInfo"));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!imageUrl) return notifyError(t("notifyErrorImage"));

      if (data.originalPrice < data.price) {
        setIsSubmitting(false);
        return notifyError(t("notifyErrorSalePrice"));
      }
      if (!defaultCategory[0]) {
        setIsSubmitting(false);
        return notifyError(t("notifyErrorProductCategory"));
      }
      ...
      if (updatedId) {
        ...
        if (res) {
          if (isCombination) {
            ...
            setIsBasicComplete(true);
            setIsSubmitting(false);
            handleProductTap(t("Combination"), true);
          } else {
            setIsUpdate(true);
        ...
        if (
          tapValue === t("Combination") ||
          (tapValue !== t("Combination") && !isCombination)
        ) {
          closeDrawer();
        }
      } else {
        ...
        if (isCombination) {
          ...
          setValue("warranty", res.warranty);
  ...
  useEffect(() => {
    if (!isDrawerOpen) {
      ...
      handleProductTap(t("BasicInfo"), true);
      setResData({});
      setValue("sku");
      setValue("title");
      setValue("slug");
  ...
  const handleGenerateCombination = () => {
    if (Object.keys(values).length === 0) {
      return notifyError(t("notifyErrorVariant"));
    }
  ...
  const handleRemoveVariant = (vari, ext) => {
    ...
    swal({
      title: `Are you sure to delete this ${ext ? "Extra" : t("Combination")}!`,
      text: `(If Okay, It will be delete this ${ext ? "Extra" : t("Combination")
        })`,
      icon: "warning",
      buttons: true,
      dangerMode: true,
  ...
  const handleProductTap = (e, value, name) => {
    ...
    if (value) {
      if (!value)
        return notifyError(`${t("PleaseSaveProductBeforeAddingCombinations")}`);
    } else {
      if (!isBasicComplete)
        return notifyError(`${t("PleaseSaveProductBeforeAddingCombinations")}`);
    }
  ...
  const handleQuantityPrice = (value, name, id, variant) => {
    if (name === "price" && Number(variant.originalPrice) < Number(value)) {
      ...
      notifyError(t("notifyErrorSalePrice"));
