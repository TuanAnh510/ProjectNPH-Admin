...
const OrderServices = {
  getAllOrders: async ({
    ...
    const statusR = statusRent !== null ? statusRent : "";
    const rents = rent !== null ? rent : "";

    return requests.get(
      `/orders?customerName=${searchName}&status=${searchStatus}&day=${searchDay}&page=${page}&limit=${limit}&startDate=${startD}&endDate=${endD}&statusRent=${statusR}&rent=${rents}`,
      body,
      headers
    );
  },
