      ...
      <TableBody>
        {products?.map((product, i) => (
          <TableRow key={i + 1}>
            ...
            <TableCell>
              <span className="text-sm font-semibold">
                {Intl.NumberFormat("vi-VN", {
                  style: "currency",
                  currency: currency,
                }).format(Number(product?.prices?.originalPrice))}
              </span>
            </TableCell>
            ...
            <TableCell>
              <span className="text-sm font-semibold">
                {Intl.NumberFormat("vi-VN", {
                  style: "currency",
                  currency: currency,
                }).format(Number(product?.prices?.price))}
              </span>
            </TableCell>
            ...
            <TableCell className="flex justify-center text-gray-400 hover:text-blue-600 mt-2">
              <Link to={`/productsComment/${product?._id}`}>
                <Tooltip Icon={FiEdit} title={t("DetailsBlog")} bgColor="red" />
              </Link>
            </TableCell>
