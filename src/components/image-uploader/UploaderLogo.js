...
const UploaderLogo = ({ setImageLogoUrl, imageLogoUrl, logo, folder }) => {
  ...
  useEffect(() => {
    ...
    if (files) {
      files.forEach((file) => {
        if (
          logo &&
          imageLogoUrl?.length + files?.length >
        ...
        if (logo) {
          const result = imageLogoUrl?.find(
        ...
        axios({
          ...
          .then((res) => {
            ...
            setLoading(false);
            if (logo) {
              setImageLogoUrl((imgUrl) => [...imgUrl, res.data.secure_url]);
            } else {
              setImageLogoUrl(res.data.secure_url);
            }
          })
          .catch((err) => {
  ...
  const handleRemoveImage = async (img) => {
    try {
      ...
      if (logo) {
        const result = imageLogoUrl?.filter((i) => i !== img);
        setImageLogoUrl(result);
      } else {
        setImageLogoUrl("");
      }
    } catch (err) {
        ...
        {logo ? (
          ...
          <div className="relative">
            ...
            <button
              ...
              onClick={() => handleRemoveImage(imageLogoUrl)}
            >
              <FiXCircle />
            </button>
          </div>
