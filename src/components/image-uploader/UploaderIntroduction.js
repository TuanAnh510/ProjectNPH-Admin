...
const UploaderIntroduction = ({
  ...
  const { getRootProps, getInputProps, fileRejections } = useDropzone({
    accept: "image/*",
    multiple: introtruction ? true : false,
    maxSize: 5000000,
  ...
  useEffect(() => {
    ...
    if (files) {
      files.forEach((file) => {
        if (
          introtruction &&
          imageIntroUrl?.length + files?.length >
        ...
        if (introtruction) {
          const result = imageIntroUrl?.find(
        ...
        axios({
          ...
          .then((res) => {
            ...
            setLoading(false);
            if (introtruction) {
              setImageIntroUrl((imgUrl) => [...imgUrl, res.data.secure_url]);
            } else {
              setImageIntroUrl(res.data.secure_url);
            }
          })
          .catch((err) => {
  ...
  const handleRemoveImage = async (img) => {
    try {
      ...
      setLoading(false);
      notifyError(res.result === "ok" ? "Xóa ảnh thành công!" : res.result);
      if (introtruction) {
        const result = imageIntroUrl?.filter((i) => i !== img);
        setImageIntroUrl(result);
      } else {
        setImageIntroUrl("");
      }
    } catch (err) {
        ...
        {introtruction ? (
          ...
          <div className="relative">
            ...
            <button
              ...
              onClick={() => handleRemoveImage(imageIntroUrl)}
            >
              <FiXCircle />
            </button>
          </div>
...
export default UploaderIntroduction;
